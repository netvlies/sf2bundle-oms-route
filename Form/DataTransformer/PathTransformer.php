...
class PathTransformer implements DataTransformerInterface
{
    ...
    public function __construct($omsConfig)
    {
     ...
     */
    public function transform($redirectRoute)
    {
        if(is_null($redirectRoute)){
            return null;
        }

        $path =  str_replace($this->omsConfig->getRoutingRoot(), '', $redirectRoute->getPath());
        $redirectRoute->setPath($path);
